...
void TrajectoryCustomPlotWindow::setTrajectoryGroup(std::shared_ptr<ArmMotionProfile> group)
{
	BasePlotWindow::setTrajectoryGroup(group);
	clear();

	plotLayout()->addElement(0, 0, new QCPTextElement(this, group->path()->name()));
...
double TrajectoryCustomPlotWindow::getPointValue(const Pose2dTrajectory& pt, const QString& node)
{
	double ret = 0.0;

	if (node == ArmMotionProfile::EPositionName) {
		ret = pt.position();
	}
	else if (node == ArmMotionProfile::EVelocityName) {
		ret = pt.velocity();
	}
	else if (node == ArmMotionProfile::EAccelerationName) {
		ret = pt.accel();
	}
	else if (node.startsWith(ArmMotionProfile::APositionName)) {
		QString numstr = QString(node.mid(strlen(ArmMotionProfile::APositionName) + 1));
		int num = numstr.toInt();
		ret = pt.angles().at(num);
	}
	else if (node.startsWith(ArmMotionProfile::AVelocityName)) {
		QString numstr = QString(node.mid(strlen(ArmMotionProfile::AVelocityName) + 1));
		int num = numstr.toInt();
		ret = pt.velocities().at(num);
	}
	else if (node.startsWith(ArmMotionProfile::AAccelerationName)) {
		QString numstr = QString(node.mid(strlen(ArmMotionProfile::AAccelerationName) + 1));
		int num = numstr.toInt();
		ret = pt.aAccel().at(num);
	}
	return ret;
}
...
void TrajectoryCustomPlotWindow::insertNode(const QString& node)
{
	...
	QVector<Pose2dTrajectory> traj = group()->trajectory();
	if (traj.count() == 0)
		return;
	...
	xAxis->setRange(0.0, traj.at(traj.count() - 1).time());
	...
	for (int i = 0; i < traj.count(); i++) {
		const Pose2dTrajectory& pt = traj.at(i);
		double time = pt.time();
		double value = getPointValue(pt, node);
